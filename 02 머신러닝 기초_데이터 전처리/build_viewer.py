...
"""
build_viewer.py
- 머신러닝 기초 데이터 전처리 ipynb 파일과 25개의 AI 해설 마크다운 노트를
...
NOTEBOOK_PATH = os.path.join(WORKSPACE_DIR, "02. 머신러닝 기초_데이터 전처리.ipynb")
OUTPUT_HTML_PATH = os.path.join(WORKSPACE_DIR, "index02.html")
...
def load_markdown_files():
    ...
    for fpath in md_files:
        ...
        if "215346" in fname and func_name == "quantile":
            display_label = "quantile (Q1, 0.25)"
            detail_badge = "하위 25% 분위수 (Q1)"
        elif "215521" in fname and func_name == "quantile":
            display_label = "quantile (Q3, 0.75)"
            detail_badge = "상위 25% 분위수 (Q3)"
        elif "215911" in fname and func_name == "sns.histplot":
            display_label = "sns.histplot (원본)"
            detail_badge = "변환 전 히스토그램"
        elif "220041" in fname and func_name == "sns.histplot":
            display_label = "sns.histplot (로그변환)"
            detail_badge = "로그 변환 후 히스토그램"
        elif "df.describe" in fname and func_name == "df.describe":
            display_label = "df.describe"
    ...
    steps_def = [
        {
            ...
            "badge": "준비 단계",
            "title": "라이브러리 Import",
            "summary": "Pandas, NumPy, Matplotlib, Seaborn, Scikit-learn 등 데이터 전처리 및 시각화 필수 패키지 로드",
            "cell_indices": [0, 1]
        },
        {
            "id": "step1",
            "number": 1,
            "badge": "1단계",
            "title": "데이터 탐색 및 문제 진단 (Data Exploration)",
            "summary": "Pima Indians 당뇨병 데이터셋 로드, 컬럼 구조 확인(df.info), 기초 통계량 진단(df.describe)",
            "cell_indices": [2, 3, 4, 5]
        },
        {
            "id": "step2",
            "number": 2,
            "badge": "2단계",
            "title": "결측치 처리 (Handling Missing Values)",
            "summary": "혈압, 포도당 등 생체 수치에서 '0' 값을 NaN으로 변환하고 SimpleImputer(평균값)로 대치",
            "cell_indices": [6, 7, 8]
        },
        {
            "id": "step3",
            "number": 3,
            "badge": "3단계",
            "title": "스케일 조정 (Feature Scaling)",
            "summary": "타깃 변수 제외 후 StandardScaler를 적용하여 평균 0, 분산 1 표준정규분포로 스케일링",
            "cell_indices": [9, 10]
        },
        {
            "id": "step4",
            "number": 4,
            "badge": "4단계",
            "title": "범주형 인코딩 (Categorical Encoding)",
            "summary": "pd.cut을 활용한 나이대 구간화(Binning) 및 pd.get_dummies / LabelEncoder 인코딩 실습",
            "cell_indices": [11, 12, 13, 14, 15, 16]
        },
        {
            "id": "step5",
            "number": 5,
            "badge": "5단계",
            "title": "이상치 처리 (Outlier Handling)",
            "summary": "IQR(사분위범위) 계산을 통한 이상치 경계(Lower/Upper Bound) 설정 및 정상 데이터 필터링",
            "cell_indices": [18, 19]
        },
        {
            "id": "step6",
            "number": 6,
            "badge": "6단계",
            "title": "분포 변환 (Distribution Transformation)",
            "summary": "한쪽으로 치우친(Skewed) 인슐린 수치를 np.log1p 로그 변환 후 Seaborn 듀얼 서브플롯으로 비교 시각화",
            "cell_indices": [20, 21]
        }
    ]
...
def map_notes_to_steps(steps, notes):
    """
    마크다운 노트를 해당 단계(Step)에 지능적으로 매핑
    """
    step_keywords = {
        "step0": [],
        "step1": ["pd.read_csv", "df.info", "df.describe"],
        "step2": ["replace", "SimpleImputer", "imputer.fit_transform"],
        "step3": ["df.drop", "StandardScaler", "scaler.fit_transform", "pd.DataFrame", "X_scaled_df.describe"],
        "step4": ["pd.cut", "head"],
        "step5": ["quantile"],
        "step6": ["df.copy", "np.log1p", "plt.subplots", "sns.histplot", "ax1.set_title", "ax2.set_title", "plt.tight_layout", "plt.show"]
    }

    # All 25 notes supported
    ...
    for n in notes:
        ...
        # All notes mapped cleanly

        mapped = False
        # Exact match first
        for step_id, kw_list in step_keywords.items():
            if func in kw_list:
    ...
    for s in steps:
        ...
        for c in s["cells"]:
            ...
            for out in c["outputs"]:
                if out["type"] == "stream":
                    ...
                    outputs_html.append(f'<div class="output-item html-table-output"><div class="output-type-tag">DataFrame Display</div>{out["content"]}</div>')
                elif out["type"] == "image":
                    outputs_html.append(
        ...
        if step_notes:
            ...
            notes_column_html.append(
                ...
                f'<h3>데이터 전처리 핵심 라이브러리 안내</h3>'
                f'<p>이 단계에서는 데이터 전처리 및 탐색적 데이터 분석(EDA)에 필요한 파이썬 필수 라이브러리들을 임포트합니다.</p>'
                f'<div class="lib-cards-grid">'
                f'<div class="lib-card"><strong>pandas</strong>: 2차원 표(DataFrame) 데이터 구조 처리 및 조작</div>'
                f'<div class="lib-card"><strong>numpy</strong>: 고성능 다차원 수치 배열 계산 및 통계 연산</div>'
                f'<div class="lib-card"><strong>matplotlib.pyplot</strong>: 정적/동적 기본 차트 및 플롯 시각화</div>'
                f'<div class="lib-card"><strong>seaborn</strong>: 통계적 데이터 시각화 라이브러리 (히스토그램, KDE 등)</div>'
                f'<div class="lib-card"><strong>sklearn.impute.SimpleImputer</strong>: 평균값/중앙값 등 결측치 대치 도구</div>'
                f'<div class="lib-card"><strong>sklearn.preprocessing.StandardScaler</strong>: 평균 0, 표준편차 1 표준화 스케일러</div>'
                f'</div>'
                f'</div>'
                f'</div>'
            )
...
<head>
  ...
  <title>머신러닝 기초: 데이터 전처리 인터랙티브 듀얼 뷰어</title>
    ...
    <div class="header-main-bar">
      <div class="brand-wrap">
        ...
        <div class="brand-titles">
          <h1>머신러닝 기초: 데이터 전처리 (Data Preprocessing)</h1>
          <p>
      ...
      <div class="header-actions">
        ...
        <div class="search-box-wrap">
          ...
          <input type="text" id="global-search" class="search-input" placeholder="함수 검색 (ex: quantile, cut...)" oninput="filterFunctions(this.value)" />
        </div>
...
def main():
    print("머신러닝 데이터 전처리 인터랙티브 듀얼 뷰어 빌더 시작...")
