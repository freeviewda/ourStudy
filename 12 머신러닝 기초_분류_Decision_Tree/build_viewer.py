...
# -*- coding: utf-8 -*-
"""
build_viewer.py for 12 머신러닝 기초_분류_Decision_Tree
- 머신러닝 기초 의사결정나무(Decision Tree) 분류 ipynb 파일과 39개의 AI 해설 마크다운 노트를
...
NOTEBOOK_PATH = os.path.join(WORKSPACE_DIR, "12_머신러닝_기초_분류_Decision_Tree.ipynb")
OUTPUT_HTML_PATH = os.path.join(WORKSPACE_DIR, "index12.html")
    ...
    steps_def = [
        {
            ...
            "summary": "NumPy, Pandas, Matplotlib, Seaborn 라이브러리와 Scikit-Learn의 의사결정나무(DecisionTreeClassifier), 트리 시각화(plot_tree), 분류 성능 평가지표 패키지 로드",
            "cell_indices": [2, 3]
        },
        {
            "id": "step1",
            ...
            "summary": "피마 인디언 당뇨병 데이터셋(Pima Indians Diabetes) 로드 및 독립변수(X)와 종속변수(y: outcome) 분리 후, 클래스 비율 유지를 위한 stratify=y 옵션으로 학습/테스트(80:20) 분할",
            "cell_indices": [4, 5, 6]
        },
        {
            "id": "step2",
            "number": 2,
            "badge": "2단계",
            "title": "의사결정나무 모델 학습 (DecisionTreeClassifier Training)",
            "summary": "과적합(Overfitting) 방지를 위해 max_depth=4(최대 깊이)와 min_samples_leaf=10(리프 노드 최소 샘플) 사전 가지치기 하이퍼파라미터를 설정하고 지니 계수(gini) 분할 기준으로 모델 학습",
            "cell_indices": [7, 8]
        },
        {
            "id": "step3",
            "number": 3,
            "badge": "3단계",
            "title": "Tree 구조 시각화 (Decision Tree Structure Visualization)",
            "summary": "plot_tree 함수를 활용하여 피처명(X.columns)과 클래스명(정상/당뇨)을 매핑하고, 불순도와 다수 클래스에 따라 색상이 칠해진 트리 다이어그램 시각화",
            "cell_indices": [9, 10]
        },
        {
            "id": "step4",
            "number": 4,
            "badge": "4단계",
            "title": "모델 성능 평가 (Accuracy & Classification Report)",
            "summary": "테스트 데이터(X_test)에 대한 모델 예측을 수행하고 정확도(Accuracy: 79.87%) 및 정밀도, 재현율, F1-Score를 포함한 분류 성능 리포트 산출",
            "cell_indices": [11, 12]
        },
        {
            "id": "step5",
            "number": 5,
            "badge": "5단계",
            "title": "특성 중요도 수치 산출 (Feature Importances Calculation)",
            "summary": "학습된 모델의 dt_clf.feature_importances_ 속성을 zip과 X.columns로 결합하여 당뇨병 분류 예측에 각 피처가 기여한 불순도 감소 중요도를 수치로 확인",
            "cell_indices": [13]
        },
        {
            "id": "step6",
            "number": 6,
            "badge": "6단계",
            "title": "특성 중요도 시각화 (Feature Importances Barplot)",
            "summary": "Seaborn의 sns.barplot을 활용하여 산출된 특성 중요도를 직관적인 가로 막대 그래프로 시각화하고 분류에 가장 결정적인 핵심 피처(Glucose 등) 분석",
            "cell_indices": [14]
        }
    ]
...
def map_notes_to_steps(steps, notes):
    ...
    step_file_patterns = {
        "step0": [],
        "step1": [
            "pd.read_csv_해설_20260923_050915",
            "df.drop_해설_20260923_050937",
            "train_test_split_해설_20260923_051115",
            "X_train.shape_해설_20260923_051141",
            "X_test.shape_해설_20260923_051150"
        ],
        "step2": [
            "DecisionTreeClassifier_해설_20260923_051413",
            "dt_clf.fit_해설_20260923_051502"
        ],
        "step3": [
            "plt.figure_해설_20260923_051536",
            "plot_tree_해설_20260923_051549",
            "plt.show_해설_20260923_051600"
        ],
        "step4": [
            "dt_clf.predict_해설_20260923_051637",
            "accuracy_score_해설_20260923_051654",
            "classification_report_해설_20260923_051803"
        ],
        "step5": [
            "X.columns_해설_20260923_051836",
            "dt_clf.feature_importances__해설_20260923_051911",
            "zip_해설_20260923_051931"
        ],
        "step6": [
            "plt.figure_해설_20260923_052115",
            "sns.barplot_해설_20260923_052128",
            "plt.title_해설_20260923_052138",
            "plt.show_해설_20260923_052146"
        ]
    }
...
def generate_viewer_html(steps, notes):
    """
    모든 데이터와 UI/UX 요소를 결합하여 최종 index12.html 생성
    """
    steps_nav_html = []
    for s in steps:
    ...
    for s_idx, s in enumerate(steps):
        ...
        if step_notes:
            ...
            notes_column_html.append(
                ...
                f'<h3>의사결정나무(Decision Tree) 분류 필수 라이브러리 안내</h3>'
...
"""
    ...
    print(f"성공적으로 index12.html 파일이 생성되었습니다: {OUTPUT_HTML_PATH}")
    print(f"   - 총 단계: {len(steps)}개")
    print(f"   - 총 해설 노트: {len(notes)}개")


def main():
    print("머신러닝 의사결정나무(Decision Tree) 분류 인터랙티브 듀얼 뷰어 빌더 시작...")
